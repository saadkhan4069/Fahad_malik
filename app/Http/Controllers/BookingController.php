...
class BookingController extends Controller
{
    public function index()
    {
        ...
        $bookings = $company->bookings()->with(['shipment', 'invoices'])->latest()->paginate(10);
    ...
    public function getBookingsData(Request $request)
    {
        ...
        $query = $company->bookings()->with(['shipment', 'invoices']);
        ...
        $totalRecords = $company->bookings()->count();
    ...
    private function getActionButtons($booking)
    {
        ...
        $buttons .= '<a href="' . route('bookings.show', $booking) . '" class="btn btn-sm btn-outline-primary" title="View"><i class="fas fa-eye"></i></a>';
        
        // Edit button - Always visible
        $buttons .= '<a href="' . route('bookings.edit', $booking) . '" class="btn btn-sm btn-primary" title="Edit"><i class="fas fa-edit"></i></a>';
        
        if ($booking->status == 'pending') {
            $buttons .= '<form method="POST" action="' . route('bookings.confirm', $booking) . '" style="display: inline;">';
            $buttons .= csrf_field();
    ...
    public function create()
    {
        ...
        return view('bookings.create', compact('services', 'countries', 'currencies', 'units', 'branches'));
    }
    ...
    public function store(Request $request)
    {
        try {
            ...
            \Log::info('Booking form submitted with data:', $request->all());
        ...
        // Get dimensions and calculate volume weight
        $length = (float)($request->length ?? 10);
        $width = (float)($request->width ?? 10);
        $height = (float)($request->height ?? 10);
        ...
        // Generate CN Number (Consignment Note Number)
        $cnNumber = $company->id . str_pad(rand(1, 99999), 5, '0', STR_PAD_LEFT);
        ...
        try {
            $booking = Booking::create([
                ...
                'cn_number' => $cnNumber,
                'status' => 'confirmed', // Auto-confirm like user bookings
                ...
                'package_description' => $request->package_description ?? 'Package',
            ...
            return redirect()->route('bookings.show', $booking)->with('success', 'Booking created and confirmed successfully!');
            ...
            $errorMessage = 'Failed to create booking. Please check your input and try again.';
    ...
    public function show(Booking $booking)
    {
        ...
        $booking->load(['shipment', 'invoices', 'company']);
        ...
        return view('bookings.show', compact('booking', 'shipperCountry', 'consigneeCountry'));
    }
    ...
    public function confirm(Booking $booking)
    {
        ...
        $shipment = Shipment::create([
            ...
            'destination_city' => 'Destination City', // You can extract from address
        ...
        return redirect()->route('bookings.show', $booking)->with('success', 'Booking confirmed and shipment created!');
    }
