...
<head>
  <meta charset="utf-8">
  <title>Proforma Invoice - {{ $booking->booking_number ?? 'INV-001' }}</title>
...
<body>
  <div class="page">
    ...
    <div class="hdr">
      <div class="hdr-left">
      <img style="margin-top: -32px;" class="logo" src="../../../storage/company/logos/EUF6Xc4fXfp5oIew8OM7SWMnwICEvoPT16rqUkfR.jpg" alt="AdEx Logo">
      
        <!-- @if($booking->company && $booking->company->logo)
          <img class="logo" src="{{ Storage::url($booking->company->logo) }}" alt="AdEx Logo">
        @elseif(file_exists(public_path('images/logo.svg')))
          <img class="logo" src="{{ asset('images/logo.svg') }}" alt="AdEx Logo">
        @elseif(file_exists(public_path('images/logo.png')))
          <img class="logo" src="{{ asset('images/logo.png') }}" alt="AdEx Logo">
           @elseif(file_exists(public_path('images/logo.jpg')))
          <img class="logo" src="{{ asset('images/logo.jpg') }}" alt="AdEx Logo">
        @else
...
<div><strong>Phone:</strong> 03391231239

<div><strong>Email:</strong>adexworldwideexpress@gmail.com </div></div>
        </div>
      </div>
    </div>
    ...
    <div class="twocol mb12">
      ...
      <div class="box">
        <table class="grid">
          ...
          <tr>
            ...
            <td class="val">
              {{ $booking->shipper_name ?? 'MUHAMMAD SHOAIB ISMAIL' }}<br>
              {{ $booking->shipper_address ?? 'H NO L-582 SEC 5, A-4 AREA NORTH KARACHI' }}
            </td>
          </tr>
          <tr><td class="lbl">City</td><td class="val">{{ $booking->shipper_city ?? 'KARACHI' }}</td></tr>
          <tr>
            <td class="lbl">Country</td>
            <td class="val">
              <div class="split">
                <div class="l">{{ $shipperCountry->name ?? $booking->shipper_country ?? 'PAKISTAN' }}</div>
                <div class="mid">CNIC</div>
                <div class="r">{{ $booking->shipper_cnic ?? '42101-0417726-5' }}</div>
              </div>
            </td>
          </tr>
          <tr>
            ...
            <td class="val">
              <div class="split">
                <div class="l">{{ $booking->shipper_phone ?? '3100034399' }}</div>
                <div class="mid">NTN No.</div>
                <div class="r">{{ $booking->shipper_ntn ?? '' }}</div>
              </div>
            </td>
          </tr>

          <tr><td class="lbl">Consignee</td><td class="val">{{ $booking->consignee_name ?? 'MR ZULFI' }}</td></tr>
          <tr><td class="lbl">Attention</td><td class="val">{{ $booking->consignee_attention ?? '' }}</td></tr>
          <tr>
            <td class="lbl">Address</td>
            <td class="val">{{ $booking->consignee_address ?? 'C18, NAJMAT MARAFID TOWER, FLAT NO 703, 7TH FLOOR.. REEM ISLAND, ABU DHABI, UAE' }}</td>
          </tr>
          <tr><td class="lbl">City</td><td class="val">{{ $booking->consignee_city ?? 'ABU DHABI' }}</td></tr>
          <tr>
            <td class="lbl">Country</td>
            <td class="val">
              <div class="split">
                <div class="l">{{ $consigneeCountry->name ?? $booking->consignee_country ?? 'UNITED ARAB EMIRATES' }}</div>
                <div class="mid">Post/Zip</div>
                <div class="r">{{ $booking->consignee_zip ?? '' }}</div>
              </div>
            </td>
          </tr>
          <tr><td class="lbl">Phone No</td><td class="val">{{ $booking->consignee_phone ?? '1559419961' }}</td></tr>
        </table>
      </div>
      ...
      <div class="box">
        <table class="grid">
          <tr><td class="lbl">AWB No.</td><td class="val">{{ $booking->shipment->tracking_number ?? '7610007585' }}</td></tr>
          <tr><td class="lbl">Date</td><td class="val">{{ $booking->booking_date ? $booking->booking_date->setTimezone(config('app.timezone'))->format('d/m/Y H:i') : now()->setTimezone(config('app.timezone'))->format('d/m/Y H:i') }}</td></tr>
          <tr><td class="lbl">Estimated Date</td><td class="val">{{ $booking->estimated_date ? $booking->estimated_date->format('d/m/Y') : 'N/A' }}</td></tr>
          <tr><td class="lbl">Service</td><td class="val">{{ $booking->service_type ?? 'LAST MILE UAE DELIVERY' }}</td></tr>
          <tr><td class="lbl">Origin</td><td class="val">{{ $booking->shipper_city ?? 'KARACHI' }}</td></tr>
          <tr><td class="lbl">Currency</td><td class="val">{{ $booking->goods_value_currency ?? 'USD' }}</td></tr>
          <tr><td class="lbl">Form 'E' No.</td><td class="val">{{ $booking->form_e_number ?? '' }}</td></tr>
          <tr><td class="lbl">Dox/Non-Dox</td><td class="val">{{ $booking->dox_type ?? 'NON-DOX' }}</td></tr>
          <tr><td class="lbl">Shipment Charges</td><td class="val">{{ 'PKR' }} {{ number_format($booking->shipment_charges ?? 0, 2) }}</td></tr>
          @php
            $dims = is_array($booking->dimensions) ? $booking->dimensions : (json_decode($booking->dimensions ?? '[]', true) ?: []);
            $len = $dims['length'] ?? null; $wid = $dims['width'] ?? null; $hei = $dims['height'] ?? null; $vol = $dims['vol_weight'] ?? ($dims['volumetric_weight'] ?? null);
          @endphp
          @if($len && $wid && $hei)
            <tr><td class="lbl">Dimensions (L×W×H)</td><td class="val">{{ $len }} × {{ $wid }} × {{ $hei }}</td></tr>
          @endif
          @if($vol)
            <tr><td class="lbl">Volumetric Weight</td><td class="val">{{ $vol }}</td></tr>
          @endif
        </table>
      </div>
    </div>
    ...
    <table class="items">
      ...
      <tbody>
        @if(isset($booking->invoice_items) && count($booking->invoice_items))
          @foreach($booking->invoice_items as $i=>$item)
            <tr>
              <td>{{ $i+1 }}</td>
              ...
              <td>{{ number_format((float)($item['quantity'] ?? 0) * (float)($item['rate'] ?? 0), 2) }}</td>
            </tr>
          @endforeach
        @else
          <tr>
            <td>1</td>
            <td>{{ $booking->package_description ?? 'Package' }}</td>
            <td></td>
            <td>{{ $booking->weight ?? '0.00' }}</td>
            <td>{{ $booking->hs_code ?? '' }}</td>
            <td>1.00</td>
            <td>PCS</td>
            <td>{{ number_format($booking->package_value ?? 0, 2) }}</td>
            <td>{{ number_format($booking->package_value ?? 0, 2) }}</td>
          </tr>
        @endif
      </tbody>
      <tfoot>
        <tr>
          ...
          <td class="bold">{{ number_format($booking->package_value ?? 0, 2) }}</td>
          <td class="bold">{{ number_format($booking->package_value ?? 0, 2) }}</td>
        </tr>
      </tfoot>
    </table>
  ...
  <script>
    function downloadPDF(){
      window.location.href='{{ route("invoices.proforma.pdf", $booking->id ?? 1) }}';
    }
  </script>
</body>
</html>
