  ...
  <div class="invoice-container" id="invoice">
    ...
    <div class="billing-section">
      ...
      <div class="invoice-details">
        ...
        <div class="detail-row"><span class="detail-label">Invoice Date:</span><span class="detail-value">{{ $invoice->invoice_date->format('M jS, Y') }}</span></div>
        <div class="detail-row"><span class="detail-label">Due Date:</span><span class="detail-value">{{ $invoice->due_date->format('M jS, Y') }}</span></div>
        <div class="total-due-box">
          <div class="total-due-content">
            <span class="total-due-label">TOTAL DUE</span>
            <span class="total-due-amount">{{ number_format($invoice->total_amount, 0) }}</span>
          </div>
        </div>
      </div>
    </div>
          ...
          @if(is_array($invoice->services))
            @foreach($invoice->services as $service)
              <tr>
                ...
                <td>{{ ($service['rate_piece'] ?? 0) == 0 ? '-' : number_format($service['rate_piece'], 0) }}</td>
                <td>{{ $service['adjust'] ?? 0 }}%</td>
                <td>{{ number_format($service['sub_total'] ?? 0, 0) }}</td>
              </tr>
            @endforeach
          @else
            <tr>
    ...
    <div class="totals-section">
      <div class="totals-box">
        <div class="total-row"><span class="total-label">Sub Total:</span><span class="total-value">{{ number_format($invoice->subtotal, 0) }}</span></div>
        <div class="total-row"><span class="total-label">Tax:</span><span class="total-value">{{ $invoice->tax_amount > 0 ? number_format($invoice->tax_amount, 0) : '--' }}</span></div>
        <div class="final-total-box">
          <div class="final-total-content">
            <span class="final-total-label">Total</span>
            <span class="final-total-amount">{{ number_format($invoice->total_amount, 0) }}</span>
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="bank-details">
      <h4>Bank Details</h4>
      <div class="bank-info">
        <div><strong>Title of Account:</strong> {{ $invoice->bank_title }}</div>
        <div><strong>Account Number:</strong> {{ $invoice->account_number }}</div>
        <div><strong>IBAN:</strong> {{ $invoice->iban }}</div>
