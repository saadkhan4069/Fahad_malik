    ...
    <div class="row">
        <div class="col-12">
            ...
            <div class="card shadow">
                ...
                <div class="card-body">
                    <form id="profile-form" method="POST" action="{{ route('company.profile.update') }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="email" class="form-control" id="email" name="email" 
                                           value="{{ old('email', $company->email ?? 'adexworldwideexpress@gmail.com') }}">
                                </div>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control" id="accounts_email" name="accounts_email" 
                                           value="{{ old('accounts_email', $company->accounts_email ?? 'adexworldwideexpress@gmail.com') }}">
                                    <small class="form-text text-muted">Separate the Email with Semicolon like 'xyz@mail.com;abc@mail.com'</small>
                                </div>
                                
                                <div class="form-group mb-3">
                                    <label for="company_address" class="form-label">Company Address</label>
                                    <textarea class="form-control" id="company_address" name="company_address" rows="3">{{ old('company_address', $company->address ?? 'Plot 13-A, Street#5, Sindhi Muslim Corporative Housing Society SMCHS, Near KFC, Karachi, Pakistan.') }}</textarea>
                                </div>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <select class="form-control" id="account_activity" name="account_activity">
                                        <option value="Courier / Bucket Shop" {{ old('account_activity', $company->account_activity ?? 'Courier / Bucket Shop') == 'Courier / Bucket Shop' ? 'selected' : '' }}>Courier / Bucket Shop</option>
                                        <option value="Logistics" {{ old('account_activity', $company->account_activity ?? '') == 'Logistics' ? 'selected' : '' }}>Logistics</option>
                                        <option value="Freight Forwarding" {{ old('account_activity', $company->account_activity ?? '') == 'Freight Forwarding' ? 'selected' : '' }}>Freight Forwarding</option>
                                        <option value="Other" {{ old('account_activity', $company->account_activity ?? '') == 'Other' ? 'selected' : '' }}>Other</option>
                                    </select>
                                </div>
                                ...
                                <div class="form-group mb-3">
                                    ...
                                    <input type="text" class="form-control" id="accounts_mobile" name="accounts_mobile" 
                                           value="{{ old('accounts_mobile', $company->accounts_mobile ?? '03062917233') }}">
                                </div>
                                            ...
                                            <div class="d-flex align-items-center">
                                                <div class="me-3">
                                                    @if($company->logo && Storage::disk('public')->exists($company->logo))
                                                        <img src="{{ asset('storage/' . $company->logo) }}" alt="Company Logo" class="img-thumbnail" style="width: 80px; height: 50px; object-fit: contain;" onerror="this.style.display='none'; this.parentElement.innerHTML='<div class=\'border rounded p-2 text-center\' style=\'width: 80px; height: 50px; background: #f8f9fa;\'><i class=\'fas fa-image text-muted\'></i></div>';">
                                                    @elseif($company->logo)
                                            ...
                                            <div class="d-flex align-items-center">
                                                <div class="me-3">
                                                    @if($company->cnic_front && Storage::disk('public')->exists($company->cnic_front))
                                                        <img src="{{ asset('storage/' . $company->cnic_front) }}" alt="CNIC Front" class="img-thumbnail" style="width: 60px; height: 40px; object-fit: cover;">
                                                    @elseif($company->cnic_front)
                                            ...
                                            <div class="d-flex align-items-center">
                                                <div class="me-3">
                                                    @if($company->cnic_back && Storage::disk('public')->exists($company->cnic_back))
                                                        <img src="{{ asset('storage/' . $company->cnic_back) }}" alt="CNIC Back" class="img-thumbnail" style="width: 60px; height: 40px; object-fit: cover;">
                                                    @elseif($company->cnic_back)
                                                        <div class="border rounded p-1 text-center" style="width: 60px; height: 40px; background: #f8f9fa;">
                                                            <i class="fas fa-id-card text-muted"></i>
                                                            <small class="d-block text-muted" style="font-size: 7px;">Not found</small>
                                                        </div>
                                            ...
                                            <div class="d-flex align-items-center">
                                                <div class="me-3">
                                                    @if($company->ntn_image && Storage::disk('public')->exists($company->ntn_image))
                                                        <img src="{{ asset('storage/' . $company->ntn_image) }}" alt="NTN Image" class="img-thumbnail" style="width: 60px; height: 40px; object-fit: cover;">
                                                    @elseif($company->ntn_image)
                                                        <div class="border rounded p-1 text-center" style="width: 60px; height: 40px; background: #f8f9fa;">
